            ...
            {task.addedBy && (
              <div className="flex items-center gap-2 text-sm">
                <Avatar className="h-10 w-10">
                  {task.addedByAvatar ? (
                    <img src={task.addedByAvatar} alt={task.addedBy} className="w-full h-full object-cover rounded-full" />
                  ) : (
                    <AvatarFallback className="text-sm bg-primary/10 text-primary">
                      {task.addedBy.split(' ').map(n => n[0]).join('').toUpperCase()}
                    </AvatarFallback>
                  )}
                </Avatar>
                <span className="text-muted-foreground">{task.addedBy}</span>
              </div>
            )}
          </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <SelectTrigger className="w-[110px] h-8">
                      <SelectValue>
                        <span className={`text-sm capitalize ${task.priority === 'high' ? 'text-red-500' :
